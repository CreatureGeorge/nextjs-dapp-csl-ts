...
export async function enableWallet(walletName) {
    ...
    let collateral = (walletName == 'nami') ? await wallet.experimental.getCollateral() : await wallet.getCollateral()
    let network = (networkID == 0) ? 'testnet' : (networkID == 1) ? 'mainnet' : `unknown: ${networkID}`
    ...
    unusedAddresses.forEach(addr => {unusedAddressesJSON.push(readableAddress(addr, S))});

    document.getElementById("walletBalance").innerHTML = balance
    document.getElementById("changeAddress").innerHTML = readableAddress(changeAddress, S)
    ...
    document.getElementById("unusedAddresses").innerHTML = JSON.stringify(unusedAddressesJSON, null, 2)
    document.getElementById("usedAddresses").innerHTML = readableAddress(usedAddresses[0], S)
